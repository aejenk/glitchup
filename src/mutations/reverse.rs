...
impl Mutation for Reverse {
    fn configure(&mut self, config: Box<&dyn MutConfig>) {
        ...
        let reversecfg = if let OMap(map) = &cfg["ReverseConfig"] {map} else {
            println!("not configuring REVERSE - not included.");
            return;
        };

        // Sets the Iterations range
        if let OArray(range) = &reversecfg["iterations"] {
        ...
        if let OArray(range) = &reversecfg["chunksize"] {
            if let (OInt(min), OInt(max)) = (&range[0], &range[1]) {
